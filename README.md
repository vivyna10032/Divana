...
然后浏览器打开 **http://127.0.0.1:8765**。左边是聊天（带打字机效果），
右边是四块只读内容：学习目标 / 现在的位置 / 下一步 / 最近的笔记。
底下还有个"整理成笔记"按钮——网页没有"退出时询问"那个环节，总结得有个按钮。
...
├─ web/index.html         网页版单页（聊天 + 侧边栏，内联 CSS/JS）
├─ divana/
...
- [ ] **v0.7 后半 · 完整界面** 知识库页、计划页、画像页、会话列表、定时任务
